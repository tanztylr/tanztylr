# Welcome to TanzaOS 👋

Building systems, organizing ideas, and turning "this would be cool" into deployed projects.

**Current Status**

🟢 Building systems

🟢 Learning cloud

🟢 Managing projects

🟡 Negotiating with the next project idea

---

## Active Projects

### 🚀 AuraCare Lite
A healthcare-focused application centered around accessibility, emergency preparedness, and epilepsy support workflows.

### 🎮 TheSimFile
A Sims 4 household generation tool designed to reduce decision fatigue and create richer storytelling experiences.

### 🌐 Tanza Portfolio
A collection of projects, lessons learned, and the occasional rabbit hole that turned into something useful.

---

## Main Quest

☁️ Google Cloud

📋 Project Management & Operations

📊 Dashboard Development

🚀 Expanding My Technical Toolkit

---

## Toolkit

☁️ Google Cloud

💻 VS Code

🔧 Git & GitHub

▲ Vercel

🤖 Gemini & AI Studio

📊 Excel & Google Sheets

📋 Project Coordination

⚙️ Workflow Optimization

📝 Documentation Systems

---

## Featured Repositories

🌐 [Tanza Portfolio](https://github.com/tanztylr/Tanza-Portfolio) — Personal portfolio and professional showcase

🚀 [AuraCare Lite](https://github.com/tanztylr/auracare-lite-) — Accessibility-focused healthcare web application

🎮 [TheSimFile](https://github.com/tanztylr/TheSimFile) — Sims 4 storytelling and household generation tool

---

## Side Quests

🍵 Amateur Matcha Critic

🧱 Occasionally Funds the LEGO Economy

🎮 Builder of Virtual Worlds & Questionable Sim Decisions

🍳 Experimental Recipe Scientist

🚗 Iridescent 4-Wheeled Vroom Lover

---

## Connect

## Connect

💼 [LinkedIn](https://www.linkedin.com/in/tanzataylor/)

🌐 [Portfolio](https://tanza-portfolio.vercel.app)

📧 [Email Me](mailto:tanzaneya.taylor1@gmail.com)
